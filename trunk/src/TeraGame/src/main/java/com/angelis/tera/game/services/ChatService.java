...
import com.angelis.tera.game.network.packet.server.SM_WHISP;
...
public class ChatService extends AbstractService {
    ...
    public void onPlayerChat(final Player player, final String chat, final ChatTypeEnum chatType) {
        ...
        if (!player.getController().can(RightEnum.TALK)) {
            ...
            player.getConnection().sendPacket(new SM_CHAT(player, I18nService.getInstance().translate(player.getAccount().getLocale(), "common.errors.chat.muted"), ChatTypeEnum.SYSTEM));
            return;
        }
